...
We investigate a family of quantum image encoders spanning tree, matrix-product and MERA topologies, four data encodings and several variational ansätze, and characterise the properties that discriminate between them: the scaling of entanglement generation with entangling-gate density, gradient variance as a function of qubit count, depolarizing-noise tolerance, and classical simulability.
...
We evaluate on CLEVR against classical tensor-network and parameter-matched neural baselines. The quantum tower exceeds its parameter-matched neural counterpart on the majority of heads, and exceeds its direct classical analogue, the same tree with CP-decomposed nodes in place of unitaries, on shape and material classification indicating that the unitarity constraint does not reduce representational capacity at this scale. A variant carrying no positional parameters attains parity on spatial-relation classification, establishing that the tree topology encodes position implicitly. On a binding task with class-conditional marginals equalised by construction, tensor networks reach 91–93\%, while a parameter-matched MLP reaches 94.6\%.
...
\subsubsection{Scalar Rotation Encoding (Scalar $R_y$)}
...
\end{equation}
While highly gate-efficient and NISQ-friendly, this approach confines the encoded state strictly to a single plane of the Bloch sphere, significantly underutilizing the qubit's available state space.

\subsubsection{Multi-Axis Encoding}
...
% ==========================================
% \chapter{Synthetic Data: Topological Optimization and Trainability}

% \section{Dataset and Aims}
% % - Introduce the 16x16 Synthetic Shapes dataset.
% % - Aims: Establish baseline perception, avoid barren plateaus, and resolve the core node architecture before scaling.

% To evaluate the candidate tensor network architectures, we introduce a procedurally generated Synthetic Shapes dataset of 16×16 RGB images featuring four geometric classes (circles, squares, triangles, crosses). To prevent trivial coordinate memorization, a random spatial jitter is applied alongside Gaussian noise to simulate real-world channel fluctuations. The dataset operates in three modes:\textit{color-only}, \textit{shape-only}, and \textit{overlapping} (decorrelated shape and color). The procedural generation code and visual examples are provided in Appendix \ref{appendix:synthetic_data}.

% The primary aim of this phase is to establish a rigorous perception baseline and explicitly test quantum topological trainability. A fundamental roadblock in scaling Variational Quantum Circuits is the barren plateau phenomenon \cite{barren_plateaus}, where gradients vanish exponentially as a function of qubit count and circuit depth. Before advancing to complex datasets, we must verify whether the structural design of the candidate topologies inherently mitigates this issue. This controlled, low-resolution environment allows us to rapidly monitor gradient variance, diagnose the onset of barren plateaus, and systematically resolve core architectural components—specifically the classical-to-quantum encoding strategy, the parameterized ansatz, and the classical readout mechanism—ensuring the surviving architectures are structurally viable and optimizable.
...
To empirically evaluate the theoretical frameworks defined in Chapter 3, we introduce a procedurally generated Synthetic Shapes dataset. This dataset consists of $16 \times 16$ RGB images featuring four geometric classes (circles, squares, triangles, and crosses). The dataset operates in three modes—\textit{color-only}, \textit{shape-only}, and \textit{overlapping} (decorrelated shape and color)—enabling targeted representation learning tests. The procedural generation code and visual examples are provided in Appendix \ref{appendix:synthetic_data}.
...
% TODO: this needs some work, for example it talks about good results from IQP but not HEA, why was HEA abandonded? 
...
To broadly explore the available architectural space, we executed a comprehensive combinatorial sweep across four encoding strategies (\texttt{angle}, \texttt{multi\_axis}, \texttt{amplitude}, and \texttt{zz\_map}) and three ans\"{a}tze (HEA, ALT, and IQP). 

As illustrated in Figure \ref{fig:encoding_ansatz_sweep}, this initial sweep served as a broad, exploratory survey of the theoretical landscape. It is crucial to define the strict constraints of this specific diagnostic:
\begin{enumerate}
    \item It was executed on a downsampled $8 \times 8$ dataset utilizing only a single 4-qubit quantum node, rather than the full hierarchical tree.
    \item It was evaluated using only a single initialization seed per configuration.
    \item While the x-axis measures simulated depolarizing noise, subsequent project scoping closed explicit noise tolerance as out of scope; thus, the relevant survey content is strictly the $p=0.000$ noiseless column.
\end{enumerate}

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=1.0\textwidth]{ch3/encoding_ansatz_sweep.png}
    \caption{Broad configuration sweep of 4 encodings $\times$ 3 ans\"{a}tze at the single-node ($8 \times 8$) scale using one seed per configuration. The relevant survey content is the $p=0.000$ column, demonstrating the general superiority of the \texttt{multi\_axis} encoding.}
    \label{fig:encoding_ansatz_sweep}
\end{figure}

The primary value of this single-node survey was identifying the most promising candidates for the hierarchical tree. Notably, the \texttt{multi\_axis} + IQP configuration achieved $95.3\%$ accuracy at $p=0$. However, this high metric is a direct artifact of the simplified $8 \times 8$ single-node task. To rigorously rank configurations, a vastly stricter statistical evaluation was required on the full image resolution.

\subsubsection{Rigorous Hybrid-Tree Evaluation}
To definitively resolve the optimal configuration, the top candidates from the survey were subjected to a rigorous evaluation using 21 independent random seeds on the $16 \times 16$ dataset (1024 train / 64 test). 
...
With the encoding established, we evaluated the parameterized ansatz, comparing the \texttt{strongly\_entangling} baseline against the structured \texttt{iqp} ansatz. Unlike the encoding choice, the empirical results revealed that the two ans\"{a}tze are statistically tied in peak accuracy on the hybrid tree. The IQP ansatz achieved a mean accuracy of $80.9\%$ (a realistic metric for the complex $16 \times 16$ task, correcting the inflated $95.3\%$ single-node survey result), edging out the strongly entangling ansatz at $79.3\%$. However, as shown in Figure \ref{fig:ansatz_comparison}, this $+1.6$ point difference falls strictly below the statistical resolution limit ($\text{MDE} = 4.8$).
...
Despite the statistical tie in raw accuracy, the \texttt{iqp} ansatz was selected for two critical structural reasons. First, the IQP ansatz demonstrated a tighter variance across the 21 seeds (standard deviation of $4.9$ vs $9.7$), indicating higher training stability. Second, the IQP circuit enforces a rigid inductive bias using commuting, diagonal operations, which requires significantly shallower physical gate depth.
...
Following the resolution of the internal node mechanics, we evaluated the final component of the architecture: the classical readout protocol. As theorized in Section 3.3.3, the hierarchical tensor topology aggressively traces out leaf qubits at each layer, forcing all spatial reasoning to funnel into the final root node. 
...
However, as established by the simulation constraints in Chapter 3, our investigation is strictly bounded to a 16-qubit architecture by the classical limits of statevector simulation. In a quantum tensor network, the bond dimension $\chi$ is defined by the number of qubits $n$ passed between adjacent nodes ($\chi = 2^n$). Because of our 16-qubit limit, we can only afford to pass $1$ qubit from each child block up to the parent layer. This imposes a hard, physical limit on the internal bond dimension of exactly $\chi = 2$. 
...
% This is the climax of the chapter: pitting the optimized quantum node against the classical controls.
% - State the parameter counts: 
%    * mlp_reference: 999 parameters
%    * classical_bare: 428 parameters (tuned CP rank)
%    * classical_full: 352 parameters
%    * quantum_coherent: 287 parameters (using unshared level-1 weights)
% - The Verdict: The quantum node beats the classical CP node by +9.7 points while using 33% fewer parameters, proving the unitary constraint acts as a powerful structural regularizer.
% - FIGURES: Reference model_comparison.png.

This phase represents the climax of the architectural investigation: pitting the fully optimized quantum tensor network against classical tensor and neural benchmarks to probe parameter efficiency.
For this final evaluation, the winning node configuration—the \texttt{multi\_axis} encoding paired with the \texttt{iqp} ansatz and multi-Pauli readout—was ported into a fully coherent 16-qubit QTTN. We compared this \texttt{quantum\_coherent} model against three classical baselines designed to test the specific efficacy of the tensor structure:
\begin{enumerate}
    \item \textbf{\texttt{mlp\_reference} (999 parameters):} A standard, unstructured Multi-Layer Perceptron to establish the classical accuracy ceiling of the dataset.
    \item \textbf{\texttt{classical\_bare} (428 parameters):} A classical tensor network utilizing Canonical Polyadic (CP) decomposition with a tuned rank, serving as the direct structural analog to the quantum tree.
    \item \textbf{\texttt{classical\_full} (352 parameters):} An optimized classical tensor network featuring added non-linearities and normalization.
    \item \textbf{\texttt{quantum\_coherent} (287 parameters):} Our fully coherent 16-qubit QTTN, utilizing unshared level-1 weights to maximize expressivity within its hardware constraints.
\end{enumerate}
The empirical results of this head-to-head comparison are illustrated in Figure \ref{fig:model_comparison}.
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=1.0\textwidth]{ch3/model_comparison.png}
    \caption{Final parameter efficiency comparison. The 287-parameter coherent QTTN outperforms the structurally equivalent classical tensor network despite operating with a strict parameter deficit, proving the efficacy of unitary regularization.}
    \label{fig:model_comparison}
\end{figure}
The \texttt{quantum\_coherent} model successfully demonstrated extreme parameter efficiency. Despite utilizing $33\%$ fewer parameters than the tuned \texttt{classical\_bare} CP tensor network, the quantum model achieved a $+9.7$ point increase in classification accuracy. This result provides compelling empirical evidence that the strict unitary constraint inherent to quantum circuits does not merely restrict the model; rather, it acts as a powerful, naturally occurring structural regularizer that forces the network to learn highly generalizable spatial representations with a remarkably low parameter footprint.
...
To evaluate the second stage of our progression, we derived explicit relational tasks from the CLEVR scenes (Left, Right, Front, Behind), as shown in \ref{fig:clevr_relations}. 
...
To evaluate this, we define a rigorous Compositional Binding Probe. The models are tasked with distinguishing between scenes that contain the exact same raw features, but in swapped configurations (e.g., distinguishing a [red square, blue circle] from a [blue square, red circle]), as seen in \ref{fig:clevr_binding}. 
...
The empirical results of this perception evaluation are detailed in Table \ref{tab:perception_results}. Initial diagnostics revealed that while the unstructured MLP networks converged rapidly (reaching 98\% of their final score by epoch 12), the tensor networks required an extended 90-epoch budget specifically to converge the 8-class \texttt{colour} head. 
...
This extended training exposed a unique structural dynamic inherent to multi-head networks: because all four heads share a single summed loss, the model must "trade heads" to reach equilibrium. As the quantum network trained longer to optimize colour, its performance on \texttt{shape} and \texttt{material} slightly degraded from their 30-epoch peaks.
...
While the preceding tasks established visual competency and relational geometry, neither perception nor spatial routing is inherently compositional. To probe the architectural limits that defeat contrastive Vision-Language Models, we introduce conduct a binding probe exepriment. The model is presented with two-object CLEVR composites, where each composite contains exactly one physically large object and one physically small object. The label is simply which side the large object sits on; consequently, the class-conditional marginal distributions of the raw visual features are identical by construction. The architecture must successfully bind the \texttt{size} attribute to a specific spatial coordinate to resolve the task.\footnote{The binding probe relies exclusively on the \texttt{size} attribute because the perception baseline (Table \ref{tab:perception_results}) proved it is highly learnable across all evaluated architectures (84.8\%--99.1\%). A parallel binding probe using the \texttt{shape} attribute was abandoned as confounded, because multiple baseline architectures sat at the perception floor for shape extraction, making binding mathematically impossible.}

Every architecture was evaluated against a patch-shuffled control. As detailed in Table \ref{tab:binding_results}, randomizing the spatial order of the image patches instantly collapses every classical architecture to exactly chance. This control mathematically guarantees that a naive "bag-of-features" model cannot solve the probe. The classical tensor networks successfully achieve this binding, resolving the task at 91.5\% and 93.2\%. However, they do not bind *better* than an unstructured baseline: a 683-parameter \texttt{mlp\_param\_matched} architecture reaches 94.6\% with fewer parameters than any tensor-network arm. Because the binding task effectively saturates across all functional architectures (a 4.5-point variance across the classical spectrum), it acts as a binary diagnostic of whether an architecture binds, not how well. We can therefore state plainly that within the isolated vision tower, there is no compositional advantage for tensor networks over a plain multi-layer perceptron.
...
thirteen times further. A control confirms that CLIP does tell these images
apart, so this is not simply a failure to see them at all. CLIP is therefore not
blind to how objects are arranged, but rearranging them registers as only
$7.6\%$ of the change caused by swapping the objects themselves. On the same
images, a 683-parameter network tells the two arrangements apart $94.6\%$ of the
time. This helps explain why such models behave like bags of words on ARO
\cite{yuksekgonul2023when}. The measurement describes what CLIP's own similarity
responds to, not whether the information could be extracted by other means.
...
Transitioning to a full VLM requires scaling the quad-tree to a standard $8\times8$ (64-patch) image grid. Natively, this requires a 64-qubit input layer. Because physical quantum hardware is unavailable, we rely on exact classical simulation, which scales exponentially as $\mathcal{O}(2^N)$. While simulating 16 qubits is trivial, a 64-qubit statevector requires exabytes of RAM, hitting a hard computational wall. 

Consequently, the strictly coherent quantum circuit is abandoned for this final phase. However, having rigorously established statistical parity between the unitary quantum node and the Canonical Polyadic (CP) tensor node in Chapters 4 and 5, we pivot exclusively to the classical CP-TTN. This substitution preserves the exact hierarchical quad-tree topology without suffering from statevector collapse, allowing us to scale to 64 patches and massive batch sizes to definitively prove the architecture works end-to-end.
...
However, integrating the hierarchical image TTN into a shared contrastive latent space alongside a text model required solving two distinct mathematical challenges: maintaining relational capacity during spatial contraction, and aligning disparate multi-modal scales.
...
This structural expansion provides the "expressive headroom" necessary to encode complex multi-linear interactions. As the $8\times8$ input grid contracts from 64 distinct patches down to a single root tensor, this increasing dimensionality ensures that critical spatial relationships are preserved rather than destructively pooled, ultimately projecting the holistic geometric structure into a final 512-dimensional image embedding.
...
\end{equation}
where $\tau$ is a learned temperature parameter and $N$ is the batch size.
...
Crucially, the baseline for this evaluation is \textbf{DisCoClip} \cite{discoclip}. Both DisCoViz and DisCoClip utilize the exact same compositional DisCoCat language model for text embedding; the \textit{only} variable changing is the vision encoder. DisCoClip relies on a frozen, self-attention-based CLIP Vision Transformer, whereas DisCoViz utilizes the structurally constrained CP-TTN. By holding the language model constant, any performance delta on the ARO benchmark can be strictly attributed to the structural differences between the global attention mechanism and the hierarchical tensor topology.
...
\begin{table}[hbt!]
    ...
    \begin{tabular}{l c c}
        ...
        \bottomrule
    \end{tabular}
    \caption{Compositional performance on the ARO benchmark. DisCoViz achieves a significant $+7.64\%$ and $+3.97\%$ performance gain over the DisCoClip baseline. Furthermore, DisCoViz drastically outperforms the massive BLIP foundation model on the Relation task.}
    \label{tab:aro_results}
\end{table}

The most critical comparison is between DisCoViz and the DisCoClip baseline. Because both architectures share the exact same compositional language model, the language-processing capacity is mathematically identical. DisCoViz's significant $+7.64\%$ gain in Attribution and $+3.97\%$ gain in Relation must therefore be strictly attributed to the vision backbone. This validates our core hypothesis: replacing the global, structurally invariant self-attention of the CLIP Vision Transformer with the hierarchical Canonical Polyadic Tree Tensor Network (CP-TTN) yields vastly superior geometric and spatial binding.

Furthermore, DisCoViz severely dominates standard, massive-scale foundation models like CLIP and OpenCLIP. While the massively parameterized BLIP model slightly edges out DisCoViz on Attribution (78.46\% vs 77.65\%), DisCoViz completely outperforms BLIP on the Relation task (59.78\% vs 52.90\%). This highlights a profound architectural reality: while massive parameter scale can statistically approximate attribution binding, it utterly fails to reliably capture the precise, rigorous geometric relationships that the tensor network topology naturally encodes.
...
This appendix contains the programmatic generation code for the synthetic shapes dataset used in Chapter 3 to investigate trainability and optimize the tensor network topologies. The dataset procedurally generates 16x16 images of basic geometric shapes (circle, square, triangle, cross) with random spatial jitter to prevent the models from memorizing absolute coordinate positions. 
The dataset supports three modes to test different compositional properties: \texttt{color\_only} (correlated shape and color), \texttt{shape\_only} (grayscale structural learning), and \texttt{overlapping} (decorrelated shape and color).
\begin{figure}[hbt!]
    \centering
