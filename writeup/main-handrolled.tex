...
For the QTTN, this empirical success is theoretically supported. Literature shows \cite{pesah2021_qcnn} \cite{liu2022tnbp} that hierarchical topologies avoid barren plateaus. However, the proof does not extend to 1D MPS or standard MERA networks. Their survival in this specific diagnostic is an empirical artifact of the shallow depths and bounded scales ($N \le 20$) tested. Nonetheless, at the operational scales required for our vision task, the gradient variance of all architectures remained highly stable and optimizable.
...
The ARO results above came from training on ARO's own split, so the model saw the exact kind of distractor it was later tested on. Whether compositional structure survives ordinary caption supervision is a different question. We retrain DisCoViz unchanged on COCO 2017 \cite{lin2014coco} --- 591,000 captions over 118,000 images, with no hard negatives, so only the InfoNCE term remains --- and evaluate it untuned on ARO, SugarCrepe \cite{hsieh2023sugarcrepe}, Winoground \cite{thrush2022winoground} and retrieval over 5,000 held-out images.

The transfer does not happen. Trained on COCO, DisCoViz scores $50.22\%$ on ARO Attribution and $50.06\%$ on Relation, both indistinguishable from chance. Retrieval sits at chance too $0.02\%$ R@1, so the joint embedding is not discriminative enough to rank a caption against the corpus at all. The one benchmark that moves is SugarCrepe, at $71.86\%$ against a $50\%$ baseline.

\subsection{Hard Negative Mining}

Hard negatives are the clearest difference between the two regimes (alongside the larger vocabulary size), so we built them for COCO. Using the parses of the text tower, we swap either two nouns or two adjectives bound to different nouns, mirroring ARO's construction. Each positive draws a negative with probability $\pi$, which varies the dose and nothing else.
...
ARO does not move, with Attribution and Relation staying within a couple of points of chance at every rate. Retrieval never leaves its one-in-5,000 floor. Winoground stays below its $16.67\%$ random baseline throughout, behaviour that has been observed in CLIP models too \cite{thrush2022winoground}. Only SugarCrepe is above chance, at around $72\%$, but most of its negatives replace a word rather than swap two, so a model reading captions as unordered bags can score there while failing every swap, it only serves to rule out a complete training failure.

Adding hard negatives therefore changes nothing, and none of these numbers describes a working vision--language model. The ARO result shows the architecture can be trained to resolve a particular kind of distractor when it is present in the loss, not that it acquires compositional structure from captions.
...
The selected architecture is a coherent quantum tree tensor network. Two design decisions proved important and were each established through experiments: intermediate states must pass between tree levels as qubits rather than being measured and re-encoded, and the model must read its top layer rather than a single root, because of simulation limitations rather. Common classical mechanisms like residual connections, dropout and explicit positional encoding, were tested and rejected due to lack of evidence supporting their usefulness.

\subsection{The Quantum Node}

The unitarity constraint is not a liability. On synthetic shapes the quantum tree node outperforms its unconstrained classical counterpart while using a third fewer parameters, and it ties the strongest classical tensor network in the study.
...
The scale at which the \emph{quantum} claims hold is narrow. A single small image size at depth two is the only coherent configuration trained end to end, and every noise result characterizes an individual tree node rather than the assembled tower. The classical tower scales considerably further but this introduces an asymmetry since it cannot be said which are scaling limitations inherent to both or just the classical model. These are consequences of classical simulation cost rather than of the architecture, and both, along with the training instability noted above, are addressed in the future work that follows.
