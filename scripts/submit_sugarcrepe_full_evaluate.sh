...
#   ML_CHECKPOINT=/path/to/best_model.pt qsub scripts/submit_sugarcrepe_full_evaluate.sh
...
if [ -z "$CHECKPOINT" ]; then
    ...
    echo "  ML_CHECKPOINT=/path/to/best_model.pt qsub scripts/submit_sugarcrepe_full_evaluate.sh"
    exit 1
fi

cd $PROJECT_DIR

$PYTHON -m qnlp.scripts.sugarcrepe_full.evaluate \
