...
# Phase C pi-sweep — cell: BOBCAT parser, LINEAR contractions, FROZEN CLIP
# ViT-B/32 image tower. SGE array task ID -> pi: 1->0, 2->0.1, 3->0.25,
# 4->0.5, 5->1.0. Rerun a single failed pi with e.g.
#   qsub -t 3 scripts/submit_pi_sweep_bobcat_linear_frozen.sh
# NOTE: no existing bobcat multi-caption LINEAR frozen script to base on
# (submit_coco_multi_caption_frozen.sh is the NON-linear variant) — this takes
# its env from the tree linear frozen script minus the tree-specific parts.
...
# --- Dataset: EXPLICITLY the nlc build (see submit_pi_sweep_bobcat_linear.sh
# for why — hard negatives are keyed to this dataset's text_hash values). ---
export ML_DATASET_NAME=coco_single_caption_nlc

# --- Hard negatives (Phase B). pi=0 never loads the bank. ---
...
export MLFLOW_RUN_NAME="${RUN_NAME:-pi_sweep_bobcat_linear_frozen}_pi${PI}"
...
echo "Cell: bobcat / linear / frozen"
