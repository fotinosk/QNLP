...
# --- Tree-no-type parser: versions the dataset/eval paths ---
export PARSER_VERSION=tree_no_type
# Linear model reads the same single tree dataset (no path column needed).
...
export MLFLOW_RUN_NAME="${RUN_NAME:-coco_frozen_tree_lin}"
...
echo "Non-linear: $ML_USE_NON_LINEAR_CONTRACTIONS   Frozen image tower: true"
