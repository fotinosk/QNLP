...
#$ -l tmem=12G
#$ -l h_rt=72:0:0
# 10-way sharded version of submit_generate_hard_negatives_enumerate.sh — 10
# independent SGE array tasks instead of one big job. Batch indices are GLOBAL
# (computed over the full sorted caption list) and each task only touches batches
# where bi % NUM_SHARDS == SHARD_INDEX, so all 10 tasks can safely write into the
# SAME --parts-dir concurrently with zero file-collision risk. Do NOT run this
...
# starting the other.
#
# 2 workers/task x 12G/slot = 24G/task requested (10 tasks x 24G = 240G total
...
echo "or resubmit the whole array (qsub -t 1-10 ...) — already-done batches from"
