...
# Phase C pi-sweep — cell: TREE_NO_TYPE parser, LINEAR contractions, NON-FROZEN
# (TTN image tower). SGE array task ID -> pi: 1->0, 2->0.1, 3->0.25, 4->0.5,
# 5->1.0. Rerun a single failed pi with e.g.
#   qsub -t 3 scripts/submit_pi_sweep_tree_linear.sh
# Based on submit_coco_multi_caption_tree_no_type_linear.sh with hard-negative
# env added.
...
echo "Cell: tree_no_type / linear / non-frozen"
