...
> **⚠️ NOTE STATUS UPDATE (2026-07-27):** The two NOTE blocks below (**Residual Connections** and, in part, **Classical Hybrid Shortcuts**) are **PROVISIONAL**, not binding, pending Phase 1.5 remediation. A code audit found that every experiment behind the residual rejections ran through a scalar-readout bottleneck — `self.head = nn.Linear(1, 4)` on a single root expectation value, i.e. the whole image compressed to one number, with all results pinned in a `50–57%` band against a `50.0%` single-attribute shortcut ceiling. Null results in that regime cannot distinguish "mechanism doesn't help" from "bottleneck dominates." See `llm/research_log.md` 2026-07-27 "Code Audit" and `llm/quantum_investigation_roadmap.md` **Section 7** (tasks R1, R3b, R5.4). Treat the rules below as the current default to build against, but do not cite them as settled findings until R3 completes. The same audit found the positional-encoding rejection (bullet above) is likewise provisional — task R3a.
>
