...
- [ ] **C3c — longer epoch budget. READY TO SUBMIT** (`qsub scripts/submit_c3c_long_run.sh`, array 1-9: **3 lrs × 3 seeds at 90 epochs**). Widened from the original "one promising seed" now that cluster time is free, and because **lower lrs converge more slowly** — testing only `lr=0.03` would probe the least under-trained config and be the weakest form of the experiment. **This is the LAST untested explanation for colour**: readout width is excluded (classical wins through a *narrower* head) and lr is excluded (C3b). **Settles whether colour's 27% is the epoch budget or the architecture** — the 30-epoch budget came from a 4-class synthetic task and was never re-examined for an 8-way head. **Diagnostic only**: one seed cannot be quoted as an arm, and if it shows the budget binds, the whole C3 comparison must be re-run at the longer budget so the classical arms get the same extension.
- [x] **C3b — quantum lr sweep. DONE 2026-08-01** (4 lrs × 3 seeds). `lr=0.03` stands; `lr=0.1` broken (2/3 collapsed). **The tuning asymmetry is closed and does NOT explain colour** — best colour across a 30× lr range is `36.3` (lr 0.01), still ~40 pts below `classical_full`. C3 needs no re-run. ~~READY TO SUBMIT~~ (`qsub scripts/submit_c3b_lr_sweep.sh`, array 1-12: 4 lrs × 3 seeds, ~3 h/task). ~~Dropped 2026-07-31~~ — that decision was made on **local** cost and is reversed now that the cluster is in use. It closes the one confound that makes C3's colour result uninterpretable: classical arms got a 60-config search, the quantum arm a single inherited `lr=0.03`. **Until it runs**, the colour gap must be cited only as "untuned quantum vs tuned classical, confounded" — never as a capacity or readout limit. Shape and material are unaffected either way (quantum wins *despite* the handicap). **If a better lr is found, C3 must be re-run at it**, or the asymmetry is merely inverted.
- [x] **C4** — relational task. RAN 2026-08-01, 10 seeds/arm on the cluster. ⚠️ **QUESTION C.2 IS NOT ANSWERED AND NOT CLOSED.** Both arms are at/below the 29.3% majority floor (`none` `28.4 ± 6.2`, `on_wire` `29.2 ± 7.3`), with **6/10 and 7/10 seeds collapsed**. The task is not learnable at this resolution, so the comparison is between two arms at chance — report as a **viability failure**, never as evidence about positional encoding. `ancilla2` NOT escalated (the budget rule required a resolved effect). Likeliest cause: relation crops span two objects and are much coarser than C3's — **the one place C5 route (a) may still be needed**.
