...
* **✅ INVESTIGATION CONCLUDED 2026-08-04.** Four statements the data supports, and one it does not:
  ...
  3. **The quantum tree can too, but unreliably** — `on_wire` seed 7 at `73.8` vs a 51.0 floor (~10 sd; an existence claim, valid on one seed), with 5 of 8 seeds at floor. ⚠️ That seed has *explicit* positional parameters; the implicit-only `none` arm has **no data yet**.
...
* **Outstanding**: 22 of 30 array tasks were killed (contention + a 24 h wall clock against ~23 h runtimes); being re-run for completeness at `-tc 8`, `h_rt=48:0:0`. **The only conclusion this can change is 3** — one clearly-above-floor `none` seed would extend the capability claim to implicit topology.
* **✅ DONE 2026-08-08 — the two ansatz figures are regenerated** as `results/figures/encoding_ansatz.png` and `results/figures/ansatz_comparison.png`, plotted by `make_figures.py` from the existing `r2_ansatz_results.json` (**no retraining needed** — R2 had already run the correct comparison at 21 seeds and its results sat unplotted). Originals archived to `results/superseded/` with defects documented in that README. ⚠️ **The `p_crit` contradiction below is recorded but NOT reconciled** — it blocks nothing now that neither artifact is cited, but resolve it before quoting any `p_crit` number. Original TODO retained below for the reasoning.
...
| 2026-08-04 | **C6: `quantum_on_wire`** (757 params) — **INCOMPLETE, 8 of 15 seeds** | CLEVR binding composites 32×32 (1024/512/90) | Score, last-5 mean | `54.8 ± 8.1`; **seed 7 = `73.8`** vs a 51.0 floor | **5 of 8 seeds at floor.** Seed 7 is ~10 binomial sd above chance → the quantum tree **can** bind (existence claim, valid on one seed) but reaches it rarely. Best seed is still 17.7 pts below `classical_bare`'s *mean*. `quantum_none` produced **no data** — all 15 tasks killed. |
