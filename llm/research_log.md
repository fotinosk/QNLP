...
* **Current Focus (as of 2026-08-01)**: **Phase 2 (CLEVR): C0, C1, C2, C3, C3b, C5 DONE. C4 REOPENED — its "task not learnable" verdict is RETRACTED; the task is learnable (classical CP tree `49.6%`, MLP `65.4%`, floor 26.6%) and the quantum arm's 28.4% is a MODEL failure. Question C.2 remains OPEN and now needs a quantum re-run, not a data fix. C3c (longer epoch budget) is the other live experiment.** The theory phase is CLOSED — every decision-gate item in [quantum_investigation_roadmap.md](file:///Users/fotinoskyriakides/Desktop/Dev/qnlp/llm/quantum_investigation_roadmap.md) Section 7 is resolved (R1, R1b, R2, R3, R4, R5, R7 done; R6 and R8 dropped with stated reasons), figures regenerated (Section 8 Phase D), no known internal contradictions.
* **Phase 2 status**:
