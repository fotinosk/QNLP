"""Build coco_contrastive_{cups}_{train,val,test}.parquet from the
derived_cups dir produced by qnlp.preprocessing_pipelines.coco.pipeline_cups.
"""
from qnlp.constants import constants
from qnlp.core.data_engine.dataset_creator.dataset_generator import create_train_val_test_datasets
from qnlp.core.data_engine.dataset_creator.strategies.contrastive_pair import ContrastivePairStrategy

COCO_DERIVED_DIR = constants.atlases_path / "coco" / "derived_cups"
LMDB_PATH = constants.lmdb_path.parent / "sentence_mapping_cups"

if __name__ == "__main__":
    create_train_val_test_datasets(
        derived_dirs=[COCO_DERIVED_DIR],
        strategy=ContrastivePairStrategy(),
        output_name="coco_contrastive_cups",
        lmdb_path=LMDB_PATH,
    )
