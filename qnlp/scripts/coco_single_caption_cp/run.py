"""
COCO single-caption contrastive training with CP-decomposed word tensors.

Each word tensor is factored into a sum of R outer products of vectors
(CP decomposition), reducing parameters ~24x for verb-core tensors while
keeping the same einsum contraction logic.  Linear contractions only.
"""
...
from qnlp.discoviz.models.einsum_model_cp import EinsumModelCP
...
from qnlp.scripts.coco_single_caption_cp.config import ExperimentConfig
...
EXPERIMENT_NAME = "coco_single_caption_cp"
...
def run():
    ...
    text_model = EinsumModelCP(symbols, sizes, rank=cfg.cp_rank).to(device)
