"""einsum_frozen_clip on cups_reader-built diagrams.

Linear word-by-word reduction via lambeq.cups_reader: each word is a rank-2
box (Word(text, S>>S) = cod = S.l @ S), then a Cup(S, S.r) contracts adjacent
wires left-to-right. No CCG parser is involved.

This is the lower bound on text-side structure: zero compositional grammar,
just sequential reduction. Combined with our CustomMPSAnsatz the user wants
us to verify whether ANY decomposition fires (see [ansatz-instrumentation]
logs from the preprocessing pipeline).
"""
...
from qnlp.scripts.einsum_frozen_clip_cups.config import ExperimentConfig
...
EXPERIMENT_NAME = "einsum_frozen_clip_cups"
logger = setup_logger(log_name=EXPERIMENT_NAME)

DATASETS_PATH = constants.datasets_path

TRAIN_PARQUET = DATASETS_PATH / "coco_contrastive_cups_train.parquet"
VAL_PARQUET = DATASETS_PATH / "coco_contrastive_cups_val.parquet"
TEST_PARQUET = DATASETS_PATH / "coco_contrastive_cups_test.parquet"
