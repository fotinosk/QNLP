"""SugarCrepe (full, all subsets) hard-negative evaluation for a trained ContrastiveVLM checkpoint.

For each (image, true_caption, false_caption) the model is "correct" if
cosine(image, true) > cosine(image, false).

Usage:
    python -m qnlp.scripts.sugarcrepe_full.evaluate --checkpoint /path/to/best_model.pt
    python -m qnlp.scripts.sugarcrepe_full.evaluate --checkpoint /path/to/best_model.pt --batch_size 64
"""
...
logger = setup_logger(log_name="sugarcrepe_full_evaluate")
...
def evaluate(checkpoint_path: Path, batch_size: int = 128) -> dict:
    device = get_device()
    parquet = constants.datasets_path / "sugarcrepe_full_eval.parquet"
    ...
    logger.info("=== SugarCrepe (full) hard-negative accuracy ===")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate a ContrastiveVLM checkpoint on SugarCrepe (full).")
