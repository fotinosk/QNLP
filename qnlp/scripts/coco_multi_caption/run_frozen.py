"""
Multi-caption COCO contrastive training — FROZEN image tower (CLIP ViT-B/32).

Images are encoded on-the-fly with CLIP ViT-B/32 and cached in memory keyed
by image path, so only the first epoch pays the encoding cost. Subsequent
epochs are instant lookups.

Only the text model (EinsumModel) and a linear text head are trained.
Val/test loaders are deduplicated to one caption per unique image for correct
retrieval evaluation. Matches run.py in every other respect.
...
EXPERIMENT_NAME = "coco_multi_caption_frozen"
CLIP_MODEL = "ViT-B/32"
...
class CLIPImageCache:
    """Encodes images with frozen CLIP on first encounter; returns cached embeddings thereafter."""
    ...
    def __call__(self, image_paths: list[str]) -> torch.Tensor:
        ...
        return torch.stack([self._cache[p] for p in image_paths]).to(self.device)
...
def _run_epoch(
    ...
    with torch.set_grad_enabled(train):
        for batch in loader:
            ...
            if gate is not None:
                totals["nonlinear_gate"] += float(gate.detach()) * bs
...
_SC_COMPILED = _ARO_COMPILED  # SugarCREPE uses the same schema
...
def _eval_winoground_frozen(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            paths0, paths1 = batch["images_0"], batch["images_1"]  # lists of strings
...
def run() -> None:
    ...
    logger.info(
        ...
        f"Val: {len(val_ds)} rows ({len(loaders['val'].dataset)} unique images) | "
    ...
    if image_cache.embedding_dim != cfg.embedding_dim:
        raise ValueError(
            ...
            "Set ML_EMBEDDING_DIM=512 to match ViT-B/32."
        )
    ...
    with setup_mlflow_run(EXPERIMENT_NAME, params, 8080) as run:
        for epoch in range(1, cfg.max_epochs + 1):
            ...
            # Use inf sentinel when all samples were dropped — prevents 0.0 from
            # falsely looking like improvement over a real loss value.
