"""Create ARO contrastive train/val/test datasets with right-to-left contraction paths.

Identical to create_dataset.py but uses right-to-left path ordering as a
controlled suboptimal baseline. Output files: aro_{split}_rtl.parquet.
"""
...
logger = setup_logger(log_name="aro_create_dataset_rtl")
...
def run() -> None:
    ...
    for split in SPLITS:
        ...
        out_path = constants.datasets_path / f"aro_{split}_rtl.parquet"
