"""Compile the ARO atlas into CCG tensor diagrams using the tree-reader (NO_TYPE mode).
...
The Bobcat CCG trees are reused from the existing diskcache; only the
diagram conversion differs (TreeReader instead of grammatical cups).
"""
...
logger = setup_logger(log_name="aro_process_tree")
...
def run() -> None:
    ...
    logger.info("ARO tree-no-type compilation complete.")
