"""
Multi-caption COCO contrastive training with EinsumModelExpressive.

Adds two improvements over the baseline NLC model:
  - Per-symbol NLC gates with mean propagation through intermediates
  - Word bypass: mean-pooled per-symbol embeddings added before normalisation
"""
...
from qnlp.discoviz.models.einsum_model_expressive import EinsumModelExpressive
...
EXPERIMENT_NAME = "coco_multi_caption_expressive"
...
class SimpleCaptionStep:
    ...
    def __call__(self, model, batch: dict, train: bool) -> tuple[Tensor, dict]:
        ...
        symbol_gates = getattr(model.text_model, "symbol_gates", None)
        if symbol_gates is not None:
            gates = symbol_gates.detach()
            metrics["gate_mean"] = gates.mean()
            metrics["gate_std"] = gates.std()
            metrics["gate_max"] = gates.max()
...
def run():
    ...
    text_model = EinsumModelExpressive(symbols, sizes, embedding_dim=cfg.embedding_dim).to(device)
    ...
    params = {
        ...
        **image_model_hyperparams.model_dump(),
        "path_strategy": "optimal",
        "gate_strategy": "per_symbol_mean_propagation",
        "bypass": "word_mean_pool",
