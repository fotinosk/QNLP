...
from qnlp.image_tower.classical_encoders.base import Encoder
...
class DFTEncoder(Encoder):
    ...
    def visualize(self, image_path: Path, sort_by_amplitude=True):
        ...
        def update_display():
            ...
            im.set_clim(
                vmin=current_reconstruction.real.min(),
                vmax=current_reconstruction.real.max(),
            )
            ax.set_title(
                f'DFT Reconstruction: {current_component_count[0]} components '
                f'(sorted by {"amplitude" if sort_by_amplitude else "position"})'
            )
            fig.canvas.draw()
        ...
        def on_plus_clicked(event):
            ...
            if current_component_count[0] < total_components:
                u, v = (
                    u_sorted[current_component_count[0]],
                    v_sorted[current_component_count[0]],
                )
