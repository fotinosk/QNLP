...
class LookupEmbedding(nn.Module):
    ...
    @classmethod
    def from_embeddings(cls, labels: Sequence[str], embedding_list: Sequence[torch.Tensor]):
    ...
    def forward(self, labels: Union[str, Sequence[str]]) -> torch.Tensor:
        ...
        idxs_tensor = torch.tensor(idxs, dtype=torch.long, device=self.embeddings.weight.device)
    ...
    @classmethod
    def load_from_checkpoint(cls, path: str, map_location: Union[str, torch.device] = "cpu"):
        """
