...
class CPQuadUnfoldLayer(nn.Module):
    ...
    def _initialize(self):
        with torch.no_grad():
            for f in [self.factor_in, self.factor_tl, self.factor_tr, self.factor_bl, self.factor_br]:
...
class QuadTreeEncoder(nn.Module):
    def __init__(self, img_size=32, patch_size=4, in_channels=1, bond_dim=64, cp_rank=16, dropout=0.1):
        super().__init__()
...
class QuadTreeDecoder(nn.Module):
    def __init__(self, img_size=32, patch_size=4, out_channels=1, bond_dim=64, cp_rank=16, dropout_p=0.1):
        super().__init__()
        self.patch_size = patch_size
        self.num_patches_side = img_size // patch_size
        num_patches = self.num_patches_side ** 2
        ...
        self.head = nn.Sequential(
            ...
            Rearrange('b (h w) (p1 p2 c) -> b c (h p1) (w p2)', 
                      h=self.num_patches_side, w=self.num_patches_side, 
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            ...
            x = rearrange(x, 'b (h w) (h2 w2) c -> b (h h2 w w2) c', h=grid_dim, w=grid_dim, h2=2, w2=2)
            grid_dim *= 2
            
        return self.head(x)
    
class QuadTreeAutoencoder(nn.Module):
    def __init__(self, img_size=32, patch_size=4, in_channels=1, bond_dim=64, cp_rank=16, dropout=0.0):
        super().__init__()
