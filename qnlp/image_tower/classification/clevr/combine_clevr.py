...
def main():
    ...
    for arm in args.arms:
        curves, missing = load_curves(args.prefix, arm, args.seeds)
        if missing:
        ...
        collapse[arm] = {"n_collapsed": len(dead), "n_seeds": len(curves), "seed_index": [i for i, _ in dead]}
        arms_by_name[arm] = cc.summarise_heads(
            arm, curves, heads, num_params=params.get(arm, 0), img_size=args.img_size, collapse=collapse[arm]
        )
    ...
    if any(v["n_collapsed"] for v in collapse.values()):
        ...
        for arm, v in collapse.items():
            print(f"    {arm:<24} {v['n_collapsed']}/{v['n_seeds']} seeds collapsed  {v['seed_index'] or ''}")
        print(
            "    A collapsed seed did not train. It drags the mean down and inflates the variance,\n"
            "    so comparisons below may be unresolvable for reasons that have nothing to do with\n"
            "    the arms. If the rate DIFFERS between arms that is itself the finding (optimisation\n"
            "    stability, not accuracy); if it is similar in both, fix the optimiser before reading\n"
            "    the means -- the quantum arms inherit lr=0.03 from Phase 1, where the loss had ONE\n"
            "    head rather than four summed, so the effective gradient scale here is ~4x larger."
        )
