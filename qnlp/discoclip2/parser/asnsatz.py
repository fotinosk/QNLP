...
from lambeq.ansatz.tensor import Dim, SplitTensorAnsatz
from lambeq.backend import grammar
from lambeq.backend.grammar import Cup, Ty, Word
...
class CustomMPSAnsatz(SplitTensorAnsatz):
    ...
    def __init__(
        ...
        self.split_functor = grammar.Functor(grammar.grammar, ob=lambda _, ob: ob, ar=self._split_ar)

    def _split_ar(self, _: grammar.Functor, ar: grammar.Box) -> grammar.Diagrammable:
        ...
        return grammar.Id().tensor(*boxes) >> grammar.Id().tensor(*cups[:-1])  # type: ignore[arg-type]
