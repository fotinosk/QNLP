...
def create_train_val_test_split(
    ...
    images_train, images_val_test = train_test_split(
        images, test_size=1 - train_size, random_state=random_state
    )
    images_val, images_test = train_test_split(
        images_val_test, test_size=0.5, random_state=random_state
    )

    print(
        f"Train/Val/Test split created with {len(images_train)} train, {len(images_val)} val, and {len(images_test)} test images."
    )
    ...
    print(
        f"Train/Val/Test split created with {len(train_df_vgr)} train, {len(val_df_vgr)} val, and {len(test_df_vgr)} test samples in VGR dataset."
    )
    ...
    print(
        f"Train/Val/Test split created with {len(train_df_vga)} train, {len(val_df_vga)} val, and {len(test_df_vga)} test samples in VGA dataset."
    )
...
class ProcessedARODataset(Dataset):
    def __init__(
        ...
        self.process_fn = preprocess if is_train else val_preprocess
        ...
        for _, row in self.processed_dataset.iterrows():
            self.text_map[row["caption"]] = self.remove_shape(
                (row["diagram"], row["symbols"])
            )

        valid_mask = raw_dataset["true_caption"].isin(
            self.text_map.keys()
    ...
    def __getitem__(self, idx):
        ...
        if self.return_images:
            ...
            image = self.process_fn(Image.open(img_path).convert("RGB"))
        else:
            image = img_name
