...
from qnlp.discoviz.trainers.frozen.train_aro import (
    BOND_DIM,
    EMBEDDING_DIM,
)
...
def worker_init(bond_dim, embedding_dim):
    ...
    _processor = BobcatTextProcessor(
        ccg_parser=_parser,
        ansatz=_ansatz,
...
def preprocess_aro(data_path: str, max_line_to_process: int = 4000):
    ...
    with concurrent.futures.ProcessPoolExecutor(
        max_workers=4, initializer=worker_init, initargs=(BOND_DIM, EMBEDDING_DIM)
    ) as executor:
